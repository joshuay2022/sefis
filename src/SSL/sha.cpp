...
{
	memset(out,0x36,sizeof(uint8_t)*size);
	//for(size_t i=0;i<size;++i) out[i]=(uint8_t)0x36;
}

void repeat_opad(uint8_t *out,const size_t &size)
{
	memset(out,0x5c,sizeof(uint8_t)*size);
	//for(size_t i=0;i<size;++i) out[i]=(uint8_t)0x5c;
}
...
{
	uint8_t *kkey;size_t kkey_size;
	uint8_t *ssecret;size_t ssecret_size=secret_size;

	if(key_size<=SIZE_OF_SHA_256_HASH){
		kkey_size=key_size;
		kkey=new uint8_t[kkey_size];
		memcpy_s(kkey,kkey_size,key,kkey_size);
		//for(size_t i=0;i<kkey_size;++i) kkey[i]=(uint8_t)key[i];
	}
	else{
		kkey_size=SIZE_OF_SHA_256_HASH;
		kkey=new uint8_t[kkey_size];
		calc_sha_256(kkey,key,key_size);
	}

	uint8_t *buf1=new uint8_t[kkey_size];
	uint8_t *buf_opad=new uint8_t[kkey_size];
	repeat_opad(buf_opad,kkey_size);
	buffer_xor(kkey,buf_opad,buf1,kkey_size);
	delete[]buf_opad;

	uint8_t *buf_2=new uint8_t[kkey_size];
	uint8_t *buf_ipad=new uint8_t[kkey_size];
	repeat_ipad(buf_ipad,kkey_size);
	buffer_xor(kkey,buf_ipad,buf_2,kkey_size);
	delete[]buf_ipad;
	uint8_t *buf2=new uint8_t[SIZE_OF_SHA_256_HASH];
	calc_sha_256(buf2,buf_2,kkey_size);

	ssecret = new uint8_t[ssecret_size];
	memcpy_s(ssecret,ssecret_size,(uint8_t*)secret,ssecret_size);

	uint8_t *_buf=new uint8_t[kkey_size+SIZE_OF_SHA_256_HASH+ssecret_size];
	memcpy_s(_buf,kkey_size,buf1,kkey_size);
	memcpy_s(_buf+kkey_size,SIZE_OF_SHA_256_HASH,buf2,SIZE_OF_SHA_256_HASH);
	memcpy_s(_buf+kkey_size+SIZE_OF_SHA_256_HASH,ssecret_size,ssecret,ssecret_size);

	calc_sha_256(out, _buf, kkey_size + SIZE_OF_SHA_256_HASH + ssecret_size);
...
{
	uint8_t *ui=new uint8_t[SIZE_OF_SHA_256_HASH];
	uint8_t *uiplus1=new uint8_t[SIZE_OF_SHA_256_HASH];
	memcpy_s(ui,saltSize,salt,saltSize);
	size_t temp_size=saltSize;
	for(size_t i=0;i<iter;++i){
		hmac_sha256(in,inSize,ui,temp_size,uiplus1);
		if(i==1) buffer_xor(ui,uiplus1,out,SIZE_OF_SHA_256_HASH);
		else if(i>0) buffer_xor(out,uiplus1,out,SIZE_OF_SHA_256_HASH);
		memcpy_s(ui,SIZE_OF_SHA_256_HASH,uiplus1,SIZE_OF_SHA_256_HASH);
		temp_size=SIZE_OF_SHA_256_HASH;
	}
...
bool verify_password(char* usr_password, size_t usr_size, size_t iter,uint8_t* hashed_password)
{
	...
	pbkdf2((uint8_t*)usr_password, usr_size, hashed_password, 16, iter, temp_hash);

	for (size_t i = 0; i < SIZE_OF_SHA_256_HASH; ++i) {
		if (temp_hash[i] != hashed_password[16 + i]) return false;
	}
