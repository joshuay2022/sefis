...
int encrypt_buffer(const unsigned char* in, unsigned char* out, const unsigned char* sha256_key)
{
  AES aes(256);
  unsigned int outLen;
  unsigned char *temp_out=aes.EncryptECB(in,16,sha256_key,outLen);
  memcpy_s(out,16,temp_out,16);
  delete[]temp_out;

  return 0;
}

int decrypt_buffer(const unsigned char* in, unsigned char* out, const unsigned char* sha256_key)
{
  AES aes(256);
  unsigned char *temp_out=aes.DecryptECB(in,16,sha256_key);
  memcpy_s(out,16,temp_out,16);
  delete[]temp_out;
...
unsigned char * AES::EncryptECB(const unsigned char in[], unsigned int inLen, const unsigned char key[], unsigned int &outLen)
{
...
unsigned char * AES::PaddingNulls(const unsigned char in[], unsigned int inLen, unsigned int alignLen)
{
  ...
  memcpy_s(alignIn,alignLen, in, inLen);
...
void AES::ShiftRow(unsigned char **state, int i, int n)    // shift row i on n positions
{
  ...
  memcpy_s(state[i],Nb * sizeof(unsigned char), tmp, Nb * sizeof(unsigned char));
